...
class YOLOModel(object):
    def __init__(self, verbose=0):
        ...
        self.label_list = ['product']
        self.label_list_cn = ['商品']
        #给每个类别的候选框设置一个颜色
        ...
        #模型的名称或路径
        self.weights = 'runs/train/product_detection/weights/best.pt'      # 'yolov5s.pt'
        if not os.path.exists(self.weights):
            self.weights = 'runs/train/product_detection/weights/last.pt'      # 'yolov5s.pt'
        assert os.path.exists(self.weights), f"注意：权重文件: {self.weights}不存在"
        # self.weights = 'runs/train/exp2/weights/last.pt'      # 'yolov5s.pt'
        self.source = 'images_dir'  #图片目录
        self.img_size = 864   #像素
        self.conf_thres = 0.3  #置信度, 大于这个置信度的才类别才取出来, 框变多，但是框可能不准确
